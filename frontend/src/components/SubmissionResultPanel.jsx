    ...
    <div className="rounded-xl border border-ink-100 bg-white p-5 shadow-card">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-ink-400">
            ...
            {result.is_late && (
              <span className="ml-2 font-medium text-fail">Late</span>
            )}
          </p>
        ...
        {result.status === "graded" && (
          <ScoreStamp score={result.total_score} max={100} />
        )}
      </div>
      ...
      {(result.status === "pending" || result.status === "grading") && (
        <p className="mt-4 text-sm text-ink-400">
          Your submission is being compiled and graded. This page will update
          automatically &mdash; feel free to refresh in a few seconds.
        </p>
      )}
      ...
      {result.compile_error && (
        <div className="mt-4 rounded-lg border border-fail/40 bg-fail/5 p-3">
          ...
          <pre className="mt-1 whitespace-pre-wrap font-mono text-xs text-ink-600">
            {result.compile_error}
          </pre>
        </div>
      )}
        ...
        <div className="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-2">
          ...
          <div>
            <h4 className="font-display text-base font-semibold text-ink-700">
              Test cases &mdash; {result.test_cases_passed}/
              {result.test_cases_total} passed
              <span className="ml-2 font-mono text-xs font-normal text-ink-300">
                ({result.test_case_score.toFixed(1)} pts)
              </span>
            </h4>
          ...
          <div>
            <h4 className="font-display text-base font-semibold text-ink-700">
              Code quality
              <span className="ml-2 font-mono text-xs font-normal text-ink-300">
                ({result.quality_score.toFixed(1)} pts)
              </span>
            </h4>
